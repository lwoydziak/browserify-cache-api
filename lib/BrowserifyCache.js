...
function BrowserifyCache(b, opts) {
  ...
  attachCacheDiscoveryHandlers(b, opts);
...
function attachCacheDiscoveryHandlers(b, opts) {
  ...
  function insertDepCollector() {
    b.pipeline.get('deps').push(through.obj(function(row, enc, next) {
      ...
      addCacheBlocker(b);
      updateCacheOnDep(b, dep, opts, function(err) {
...
function updateCacheOnDep(b, dep, opts, done) {
  var cache = BrowserifyCache.getCache(b);

  var file = dep.file || dep.id;
  if (typeof file === 'string') {
    for (var excluded of opts.exclude){
      if (file.match(excluded)) {
        console.log("lib/BrowserifyCache.js:152 excluding: ", file);
        done();
        return;
      }
    }
